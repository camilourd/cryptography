...
public class DES {
	...
	// Final Permutation
	public final static DESSubstitution FP = new DESSubstitution(new byte[] {
	...
	public final static DESSubstitution P = new DESSubstitution(new byte[] {
			15, 6,  19, 20,
	        28, 11, 27, 16,
	        0,  14, 22, 25,
	        4,  17, 30, 9,
	...
	public static BitArray feistel(BitArray right, BitArray subkey) {
		BitArray result = expand((BitArray) right.clone());
		result.xor(subkey);
		return P.substitute(substitute(result));
	}
	
	private static void swap(BitArray[] parts) {
		BitArray aux = parts[0];
		parts[0] = parts[1];
		parts[1] = aux;
	}
	
	private static BitArray expand(BitArray right) {
		right.add(new BitArray(16, false));
		return EP.substitute(right);
	}
	...
	private static BitArray join(BitArray left, BitArray right) {
		left.add(right);
		return left;
	}
